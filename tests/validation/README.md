...
- [x] `antenna_metrics` — Edge-fed patch antenna at resonance (2.4 GHz). Smoke-checks: directivity in 2-12 dBi, gain ≤ directivity, AR>10 dB (linear pol). `python compare_antenna_metrics.py`. **Known artifact**: front lobe at θ=0 gives D=3.89 dBi (physical, matches expected patch performance); a phantom back lobe at θ=178° gives D=4.95 dBi because the NFFT surface is open — the PEC ground plane is not included in the integration, so equivalence currents radiate spuriously into the lower hemisphere. Front-lobe metrics are trustworthy; the peak-finder picks the artifact lobe. Closing the NFFT surface is task #31. Quantitative pattern validation against EMerge demo4 is blocked by the gmsh-PLC environment issue on this Windows install.
