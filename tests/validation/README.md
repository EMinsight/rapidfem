...
- [x] `pml` — Uniaxial PML termination of WR-90 waveguide (instead of an ABC). With δmax=8, n=1.5, thickness 15mm: |S11|<0.004 across 9–11 GHz, 11/11 OK. `python compare_pml.py`. **Note**: the PML produces anisotropic-complex tensors that have triggered a PARDISO crash on Windows; the test forces faer (`RAPIDFEM_SOLVER=faer`). PARDISO compatibility for PML is a follow-up.
