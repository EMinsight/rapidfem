...
- [x] `antenna_metrics` — Edge-fed patch antenna at resonance (2.4 GHz). Smoke-checks: directivity in 2-12 dBi, gain ≤ directivity, AR>10 dB (linear pol), peak in upper hemisphere. `python compare_antenna_metrics.py`. **Known issue**: peak directivity location is at θ≈178° (essentially -z) rather than the expected broadside +z — the *magnitude* of directivity/gain/AR is correct but the angular pattern appears mirror-flipped, suggesting an outward-normal sign issue in the NFFT integration. Quantitative pattern validation against EMerge demo4 is blocked by the gmsh-PLC environment issue.
